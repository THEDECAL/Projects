...
namespace app{
	application::application(){
		...
		if(!fmngr::find(fileMngr::ACCOUNTS + "admin")){
			profile::user admin(profile::ADMIN,"admin","123","","");
		}
	}
	application* application::getRef(){
		return pApplication;
	}
		...
		do{
			...
			case REGISTRATION:{
				do{
					...
					pAccount = new profile::user(profile::USER,login,password,fname,sname);
					pAccount->save();
	...
	application* application::pApplication = new application;
	application* pApp = application::getRef();
