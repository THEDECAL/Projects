...
unsigned aborigen::choice(const string* arguments){
...
void aborigen::inputHistory(const char* line){
	...
	if(!addToHis) printf("ERROR. Dont open history file \" %s \"\n",HistoryFile.c_str());
	const time_t seconds=time(NULL);
	string _time=ctime(&seconds);
	addToHis<<'. '<<SpcToUnln(_time,_time.size()+1)<<"\t\""<<line<<"\"\n";

	if(addToHis) addToHis.close();
}
void aborigen::initFileHistory(){
	ofstream readHis(HistoryFile,ios::in);
	if(!readHis) printf("ERROR. Dont open history file \" %s \"\n",HistoryFile.c_str());
