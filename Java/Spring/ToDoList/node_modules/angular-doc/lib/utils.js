...
/**
* Walk the directory tree to locate the angulardoc.json file.
...
var getProjectData = function(dir) {
    ...
    while (dirs.length && !projectData) {
        if (dirCount++ > 5000) {
            Y.log('Scanned ' + dirCount + ' directories looking for a angulardoc.json file, something is probably wrong here..', 'error', 'angular-doc');
            process.exit(1);
        }
        ...
        dirs.forEach(function(dir) {
            ...
            try {
                ...
                names.forEach(function(name) {
                    ...
                    if (Y.Files.isFile(p)) {
                        projectData = projectData || getFileData(p, name, 'angulardoc.json');
                        // 'package.json' is used for auxilliary configuration
                        // if it's found. Formerly, it was only found if it
                        // came _before_'angulardoc.json' in the folder tree
                        // (never in the same folder).
                        // This code will find 'package.json' in the same
                        // folder as 'angulardoc.json'.
                        // If there is no 'angulardoc.json', former algorithm would
                    ...
                    if (Y.Files.isDirectory(p)) {
                        ...
                        if (name === 'node_modules') {
                            Y.log('Skipping node_modules directory while scanning for angulardoc.json', 'warn', 'angular-doc');
                            return;
                        }
                        childDirs.push(p);
                    }
                ...
                Y.log('Accessing dir (' + dir + ') threw an error', 'warn', 'angular-doc');
            }
        });
