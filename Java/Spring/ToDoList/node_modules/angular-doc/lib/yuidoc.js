...
/**
This is the __module__ description for the `AngularDoc` module.
    ...
    Y.YUIDoc.prototype = {
        ...
        parsedir: function(dir) {
            ...
            if(!Y.Lang.isArray(this.options.ignorePaths)) {
                ...
                if(globMatch === false) {
                    ...
                    Y.log('skipping directory : ' + relativePathToRoot + ' : based on glob match : ' + globMatchContent, 'info', 'angular-doc');
                }
            }
                ...
                if (ext) {
                    if (ext in self.options.extensions) {
                        ...
                        if (Y.Files.exists(fullpath)) {
                            ...
                            Y.log('File skipped: ' + fullpath, 'warn', 'angular-doc');
                        }
                    }
                }
            });
        ...
        getSelleck: function(fullpath) {
            ...
            if (self.options.selleck) {
                ...
                //Y.log('Checking for Selleck data: ' + comp, 'info', 'angular-doc');
                if (Y.Files.exists(comp)) {
                    try {
                        ...
                        Y.log('JSON parse failed on Selleck component.json file: ' + comp, 'error', 'angular-doc');
                    }
                }
            }
        },
        ...
        writeJSON: function(parser) {
            ...
            if (self.options.writeJSON) {
                ...
                /*if (Y.Files.exists(self.options.outdir) && !self.options.nodeleteout) {
                    Y.log('Found out dir, deleting: ' + self.options.outdir, 'warn', 'angular-doc');
                    rimraf.sync(self.options.outdir);
                }*/
                if (!Y.Files.exists(self.options.outdir)) {
                    Y.log('Making out dir: ' + self.options.outdir, 'info', 'angular-doc');
                    try {
                        fs.mkdirSync(self.options.outdir, 0777);
                    } catch (e) {
                        Y.log('Outdir creation failed', 'warn', 'angular-doc');
                    }
                }
        ...
        lint: function(warnings) {
            ...
            if (warnings && warnings.length) {
                code = 1;
                console.log('AngularDoc found', warnings.length, 'lint errors in your docs');
        ...
        run: function() {
            /**
             * Timestamp holder so we know when AngularDoc started the parse process.
             * @property starttime
             * @type Timestamp
             */
            Y.log('AngularDoc Starting from: ' + this.options.paths.join(','), 'info', 'angular-doc');
            ...
            /**
             * Timestamp holder so we know when AngularDoc has finished the parse process.
             * @property endtime
             * @type Timestamp
             */
            this.endtime = new Date().getTime();
            Y.log('Parsed ' + this.filecount + ' files in ' + ((this.endtime - this.starttime) / 1000) + ' seconds', 'info', 'angular-doc');
