 ...
 *
 * @property Role[] $defaultRoleInstances Default positions. The array is indexed by the role names. This property
 * is read-only.
 * @property string[] $defaultRoles Default positions. Note that the type of this property differs in getter and
...
abstract class BaseManager extends Component implements ManagerInterface
{
     ...
     * Note that these positions are applied to users, regardless of their state of authentication.
     */
     ...
     * @param string[]|\Closure $roles either array of positions or a callable returning it
     * @throws InvalidArgumentException when $positions is neither array nor Closure
    ...
    public function setDefaultRoles($roles)
    {
        if (is_array($roles)) {
            ...
            if (!is_array($roles)) {
                throw new InvalidValueException('Default positions closure must return an array');
            }
            $this->defaultRoles = $roles;
        } else {
            throw new InvalidArgumentException('Default positions must be either an array or a callable');
        }
    }

    /**
     * Get default positions
     * @return string[] default positions
     * @since 2.0.14
     */
     ...
     * @since 2.0.12
     * @return Role[] default positions. The array is indexed by the role names
     */
