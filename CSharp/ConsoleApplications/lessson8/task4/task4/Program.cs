...
пузырек.
    -- Я совершенно  уверена, что раньше его здесь не было!--
...
внезапной переменой, но радуясь, что уцелела. --А теперь--  в
сад!
...
ясно представляла себе, что когда происходило.
 ...
 --  Простите!-быстро  сказала Алиса, видя, что обидела
бедного зверька. -- Я забыла, что вы не любите кошек.
     --Не люблю кошек? --вскричала пронзительно Мышь. --А ты
бы их на моем месте любила ?
...
--  Вылезем на  берег,--сказала  Мышь тихим, дрожащим
голосом, --и я расскажу тебе мою историю.Тогда ты поймешь, за
что я ненавижу кошек и собак.
...
Король, скрестил на груди руки и, грозно нахмурив  брови,  так
...
голову!Гоните ее в шею!Подавите ее!Ущипните ее!Отрежьте ей
усы!
...
дни.";
            ...
            string[] splitText = text.Split(delimiters,StringSplitOptions.RemoveEmptyEntries);

            //Добавляю слова в словарь
            foreach (string item in splitText)
            {
                string lItem = item.ToLower();
                if (!dic.ContainsKey(lItem))
                    dic.Add(lItem, 1);
                else dic[lItem] += 1;
            }

            //Вывожу слова и их количество
            foreach (string item in dic.Keys)
                Console.WriteLine($"{item} {dic[item]}");

            //Console.Write(text);
            //List<string> list = new List<string> { "--", "!", "?", "(", ")", "'", "`", "...", ":" }.ForEach(s => text = text.Replace(s, ""));
            Console.ReadKey();
        }
    }
}
