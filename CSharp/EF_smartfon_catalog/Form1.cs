...
namespace smartfon_catalog
{
    public partial class Form1 : Form
    {
        ...
        /// <summary>
        /// Метод обновления списка телефонов
        /// </summary>
        public void UpdateDb() => db = Downloader.GetSmartfones();
        /// <summary>
            ...
            foreach (var prop in s.GetType().GetProperties())
            {
                ...
                if(i > -1)
                {
                    ...
                    if (lstBox.SelectedIndices.Count > 0)
                    {
                        ...
                        {
                            ;
                            string val = s.GetType().GetProperty(prop.Name).GetValue(s).ToString();
                            if (lstBox.Items[index].ToString() == val)
                            {
                                check = true;
                                break;
                            }
        ...
        /// <summary>
        /// Метод добавления нового смартфона
        /// </summary>
        private void btnAdd_Click(object sender, EventArgs e)
        {
            Smartfone phone = new Smartfone();
            PhoneInfo pi = new PhoneInfo(phone, PhoneInfo.Mode.Add);
            pi.ShowDialog();
            UpdateDb();
            LoadData();
        }
